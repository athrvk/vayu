...
class ScenarioLoadDriver {
    ...
    : context_ (context), db_ (db), execution_ (execution), state_ (std::move (state)),
      max_iterations_ (max_iterations), live_vus_ (state_->vus.size ()) {
    }
    ...
    [context = context_, &db = db_, state = state_, step_count = execution_.plan.steps.size (),
    vu, step_index, iteration, row] (
