...
validate_response_schemas_index (const nlohmann::json& index, size_t cap) {
    if (!index.is_object ()) {
        return "Invalid 'responseSchemas': must be an object with an 'operations' array";
    }
    const auto serialized = index.dump ().size ();
    if (serialized > cap) {
        return "Response schema index is " + std::to_string (serialized) +
        " bytes, over the limit of " + std::to_string (cap) +
        " (raise the 'maxSpecDocumentBytes' setting to allow more)";
    }
    if (auto roots = index.find ("refRoots");
        roots != index.end () && !roots->is_null () && !roots->is_object ()) {
        return "Invalid 'responseSchemas.refRoots': must be an object";
    }

    auto operations = index.find ("operations");
    if (operations == index.end () || !operations->is_array ()) {
        return "Invalid 'responseSchemas.operations': must be an array";
    }
    if (operations->size () > constants::spec_document::MAX_OPERATIONS) {
        return "Response schema index declares " + std::to_string (operations->size ()) +
        " operations, over the limit of " +
        std::to_string (constants::spec_document::MAX_OPERATIONS);
    }

    for (size_t i = 0; i < operations->size (); ++i) {
        const auto& row = (*operations)[i];
        const std::string at = "responseSchemas.operations[" + std::to_string (i) + "]";
        if (!row.is_object ()) {
            return "Invalid '" + at + "': must be an object";
        }
        for (const char* required : { "method", "path" }) {
            auto value = row.find (required);
        ...
        if (auto id = row.find ("operationId");
            ...
            return "Invalid '" + at + ".operationId': must be a string";
        }
        auto responses = row.find ("responses");
        if (responses == row.end () || !responses->is_array ()) {
            return "Invalid '" + at + ".responses': must be an array";
        }
        for (size_t j = 0; j < responses->size (); ++j) {
            const auto& response = (*responses)[j];
            const std::string response_at = at + ".responses[" + std::to_string (j) + "]";
            if (!response.is_object ()) {
                return "Invalid '" + response_at + "': must be an object";
            }
            for (const char* required : { "status", "contentType" }) {
                auto value = response.find (required);
                if (value == response.end () || !value->is_string () ||
                value->get<std::string> ().empty ()) {
                    return "Invalid '" + response_at + "." + required +
                    "': must be a non-empty string";
                }
            }
            // A schema may legally be `true` or `false` as well as an object -
            // both are valid JSON Schema, and `false` ("nothing validates
            // here") is a real thing for a document to declare.
            auto schema = response.find ("schema");
            if (schema == response.end () ||
            !(schema->is_object () || schema->is_boolean ())) {
                return "Invalid '" + response_at +
                ".schema': must be a schema object or boolean";
            }
        }
    }
    return std::nullopt;
