...
nlohmann::json variable_scope_schema () {
    return { { "type", "string" },
        { "enum", { "env", "collection", "globals" } }, { "title", "Store in" },
        { "description", "Which variable scope to write the result into." } };
}
...
ElementKind make_extract_json_kind () {
    ...
    kind.config_schema = {
        ...
        { "description", "The variable name to write the result into." } } },
        { "scope", variable_scope_schema () },
        { "default",
        { { "type", "string" }, { "title", "Fallback value" },
        { "description", "Written to the variable when there is no match." } } },
        { "matchNo",
        { { "type", "integer" }, { "title", "Which match" }, { "x-vayu-group", "advanced" },
        { "description",
        "Selects among multiple matches: 1-based index, 0 for random, "
        "-1 to write every match." } } },
        { "required",
        { { "type", "boolean" }, { "title", "Required" },
        { "description", "Fails the step's assertions when no match is found." } } } } },
...
ElementKind make_extract_regex_kind () {
    ...
    kind.config_schema = {
        ...
        { "description",
        "A $1$-style template of the match's capture groups; $0$ is "
        "the whole match." } } },
        { "field",
        { { "type", "string" }, { "enum", { "body", "headers", "url", "status" } }, { "title", "Field" },
        { "description",
        "Which part of the response to search: body, headers, url "
        "or status." } } },
        { "variable",
        { { "type", "string" }, { "minLength", 1 }, { "title", "Variable name" },
        { "description", "The variable name to write the result into." } } },
...
ElementKind make_extract_header_kind () {
    ...
    kind.config_schema = {
        ...
        { "properties",
        { { "header",
          { { "type", "string" }, { "minLength", 1 }, { "title", "Header name" },
          { "description", "The response header to read, matched case-insensitively." } } },
        { "variable",
        { { "type", "string" }, { "minLength", 1 }, { "title", "Variable name" },
        { "description", "The variable name to write the result into." } } },
        { "scope", variable_scope_schema () },
...
ElementKind make_extract_boundary_kind () {
    ...
    kind.config_schema = {
        ...
        { "properties",
        { { "leftBoundary",
          { { "type", "string" }, { "title", "Left boundary" },
          { "description", "The text immediately before the value to extract." } } },
        { "rightBoundary",
        { { "type", "string" }, { "title", "Right boundary" },
        { "description", "The text immediately after the value to extract." } } },
        { "variable",
        { { "type", "string" }, { "minLength", 1 }, { "title", "Variable name" },
        { "description", "The variable name to write the result into." } } },
        { "scope", variable_scope_schema () },
