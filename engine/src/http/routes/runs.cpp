        ...
        R"(/run/([^/]+)/report)", [&ctx](const httplib::Request& req, httplib::Response& res) {
            ...
            try {
                ...
                try {
                    ...
                    if (config.contains("mode")) config_obj["mode"] = config["mode"];
                    if (config.contains("duration")) config_obj["duration"] = config["duration"];
                    if (config.contains("rps")) config_obj["targetRps"] = config["rps"];
                    if (config.contains("targetRps")) config_obj["targetRps"] = config["targetRps"];
                    if (config.contains("concurrency"))
                        config_obj["concurrency"] = config["concurrency"];
                    if (config.contains("timeout")) config_obj["timeout"] = config["timeout"];
                ...
                json_report["latency"] = {{"min", report.latency_min},
                                          ...
                                          {"median", report.latency_p50},
                                          {"p50", report.latency_p50},
                                          {"p75", report.latency_p75},
                ...
                for (const auto& result : results) {
                    ...
                    if (!result.error.empty()) result_obj["error"] = result.error;
