...
SseParser::SseParser(EventCallback callback) : callback_(std::move(callback)) {}
...
struct EventSource::Impl {
    ...
    Impl(std::string url_, EventSourceConfig config_)
        : url(std::move(url_)), config(std::move(config_)), current_retry_ms(config.retry_ms) {}
    ...
    void run_connection() {
        ...
        while (!should_close) {
            ...
            // Reset parser
            parser = std::make_unique<SseParser>([this](SseEvent event) { dispatch_event(event); });
            ...
            // Connection timeout
            curl_easy_setopt(
                curl, CURLOPT_CONNECTTIMEOUT_MS, static_cast<long>(config.connect_timeout_ms));
