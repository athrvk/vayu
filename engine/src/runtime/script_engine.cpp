...
namespace vayu::runtime
{
    ...
    namespace
    {
        ...
        JSValue expect_not_getter(JSContext *ctx, JSValueConst this_val,
                                  int argc, JSValueConst *argv)
        {
            (void)argc;
            (void)argv;
        ...
        JSValue expect_be_getter(JSContext *ctx, JSValueConst this_val,
                                 int argc, JSValueConst *argv)
        {
            (void)argc;
            (void)argv;
        ...
        JSValue expect_equal(JSContext *ctx, JSValueConst this_val,
                             int argc, JSValueConst *argv)
        {
            if (argc < 1)
            {
        ...
        JSValue expect_exist(JSContext *ctx, JSValueConst this_val,
                             int argc, JSValueConst *argv)
        {
            auto *state = static_cast<ExpectState *>(JS_GetOpaque(this_val, expect_class_id));
            if (!state)
        ...
        JSValue expect_true(JSContext *ctx, JSValueConst this_val,
                            int argc, JSValueConst *argv)
        {
            auto *state = static_cast<ExpectState *>(JS_GetOpaque(this_val, expect_class_id));
            if (!state)
        ...
        JSValue expect_false(JSContext *ctx, JSValueConst this_val,
                             int argc, JSValueConst *argv)
        {
            auto *state = static_cast<ExpectState *>(JS_GetOpaque(this_val, expect_class_id));
            if (!state)
        ...
        JSValue expect_above(JSContext *ctx, JSValueConst this_val,
                             int argc, JSValueConst *argv)
        {
            if (argc < 1)
            {
            ...
            {
                std::string msg = state->negated
                                      ? "Expected " + std::to_string(actual) + " to not be above " + std::to_string(expected)
                                      : "Expected " + std::to_string(actual) + " to be above " + std::to_string(expected);
                return JS_ThrowTypeError(ctx, "%s", msg.c_str());
            }
        ...
        JSValue expect_below(JSContext *ctx, JSValueConst this_val,
                             int argc, JSValueConst *argv)
        {
            if (argc < 1)
            {
            ...
            {
                std::string msg = state->negated
                                      ? "Expected " + std::to_string(actual) + " to not be below " + std::to_string(expected)
                                      : "Expected " + std::to_string(actual) + " to be below " + std::to_string(expected);
                return JS_ThrowTypeError(ctx, "%s", msg.c_str());
            }
        ...
        JSValue expect_include(JSContext *ctx, JSValueConst this_val,
                               int argc, JSValueConst *argv)
        {
            if (argc < 1)
            {
        ...
        JSValue expect_have_property(JSContext *ctx, JSValueConst this_val,
                                     int argc, JSValueConst *argv)
        {
            if (argc < 1)
            {
        ...
        JSValue create_expectation(JSContext *ctx, JSValue actual)
        {
            ...
            JS_DefinePropertyGetSet(ctx, obj, to_atom,
                                    JS_NewCFunction(ctx, expect_to_getter, "to", 0),
                                    JS_UNDEFINED, 0);
            ...
            JS_DefinePropertyGetSet(ctx, obj, not_atom,
                                    JS_NewCFunction(ctx, expect_not_getter, "not", 0),
                                    JS_UNDEFINED, 0);
            ...
            JS_DefinePropertyGetSet(ctx, obj, be_atom,
                                    JS_NewCFunction(ctx, expect_be_getter, "be", 0),
                                    JS_UNDEFINED, 0);
            ...
            JS_DefinePropertyGetSet(ctx, obj, have_atom,
                                    JS_NewCFunction(ctx, expect_to_getter, "have", 0),
                                    JS_UNDEFINED, 0);
            JS_FreeAtom(ctx, have_atom);

            // Add assertion methods
            JS_SetPropertyStr(ctx, obj, "equal",
                              JS_NewCFunction(ctx, expect_equal, "equal", 1));
            JS_SetPropertyStr(ctx, obj, "eql",
                              JS_NewCFunction(ctx, expect_equal, "eql", 1));
            JS_SetPropertyStr(ctx, obj, "exist",
                              JS_NewCFunction(ctx, expect_exist, "exist", 0));
            JS_SetPropertyStr(ctx, obj, "true",
                              JS_NewCFunction(ctx, expect_true, "true", 0));
            JS_SetPropertyStr(ctx, obj, "false",
                              JS_NewCFunction(ctx, expect_false, "false", 0));
            JS_SetPropertyStr(ctx, obj, "above",
                              JS_NewCFunction(ctx, expect_above, "above", 1));
            JS_SetPropertyStr(ctx, obj, "below",
                              JS_NewCFunction(ctx, expect_below, "below", 1));
            JS_SetPropertyStr(ctx, obj, "include",
                              JS_NewCFunction(ctx, expect_include, "include", 1));
        ...
        JSValue js_pm_test(JSContext *ctx, JSValueConst this_val,
                           int argc, JSValueConst *argv)
        {
            if (argc < 2)
            {
        ...
        JSValue js_pm_expect(JSContext *ctx, JSValueConst this_val,
                             int argc, JSValueConst *argv)
        {
            if (argc < 1)
            {
        ...
        JSValue js_response_json(JSContext *ctx, JSValueConst this_val,
                                 int argc, JSValueConst *argv)
        {
            auto *data = get_context_data(ctx);
            if (!data->response)
        ...
        JSValue js_response_text(JSContext *ctx, JSValueConst this_val,
                                 int argc, JSValueConst *argv)
        {
            auto *data = get_context_data(ctx);
            if (!data->response)
        ...
        void setup_pm_response(JSContext *ctx, JSValue pm)
        {
            ...
            {
                // pm.response.code
                JS_SetPropertyStr(ctx, response, "code",
                                  JS_NewInt32(ctx, data->response->status_code));
        ...
        void setup_pm_request(JSContext *ctx, JSValue pm)
        {
            ...
            {
                // pm.request.url
                JS_SetPropertyStr(ctx, request, "url",
                                  JS_NewString(ctx, data->request->url.c_str()));
        ...
        JSValue js_pm_environment_get(JSContext *ctx, JSValueConst this_val,
                                      int argc, JSValueConst *argv)
        {
            if (argc < 1)
            {
        ...
        JSValue js_pm_environment_set(JSContext *ctx, JSValueConst this_val,
                                      int argc, JSValueConst *argv)
        {
            if (argc < 2)
            {
        ...
        void setup_pm_environment(JSContext *ctx, JSValue pm)
        {
            ...
            JS_SetPropertyStr(ctx, env, "get",
                              JS_NewCFunction(ctx, js_pm_environment_get, "get", 1));
            JS_SetPropertyStr(ctx, env, "set",
                              JS_NewCFunction(ctx, js_pm_environment_set, "set", 2));
        ...
        void setup_pm_object(JSContext *ctx)
        {
            ...
            // pm.test()
            JS_SetPropertyStr(ctx, pm, "test",
                              JS_NewCFunction(ctx, js_pm_test, "test", 2));

            // pm.expect()
            JS_SetPropertyStr(ctx, pm, "expect",
                              JS_NewCFunction(ctx, js_pm_expect, "expect", 1));
    ...
    public:
        ...
        ScriptResult execute(const std::string &script, const ScriptContext &ctx)
        {
            ...
            // Execute script
            JSValue eval_result = JS_Eval(js_ctx, script.c_str(), script.size(),
    ...
    ScriptResult ScriptEngine::execute(const std::string &script,
    ...
    ScriptEngine::ScriptEngine(const ScriptConfig &)
        : impl_(nullptr)
    {
    }
    ...
    ScriptResult ScriptEngine::execute(const std::string &,
                                       const ScriptContext &)
    {
        ScriptResult result;
        result.success = false;
    ...
    ScriptResult ScriptEngine::execute_prerequest(const std::string &,
                                                  Request &,
                                                  Environment &)
    {
        ScriptResult result;
