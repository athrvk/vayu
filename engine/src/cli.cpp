...
For more information, visit: https://github.com/vayu/vayu
    ...
    int run_via_daemon(const std::string &daemon_url, const std::string &filepath, bool verbose, bool color)
    {
        // Read request file
        std::string content;
        try
        ...
        bool is_load_test = json.contains("mode") || json.contains("iterations") || json.contains("duration");
        ...
        if (is_load_test)
        {
            ...
            {
                std::string msg = "Error starting load test (Status " + std::to_string(res->status) + ")";
                std::cerr << msg << "\n";
                std::cerr << res->body << "\n";
                vayu::utils::log_error(msg);
...
int main(int argc, char *argv[])
{
    ...
    // httplib uses its own socket implementation, but vayu::http::global_init might be needed if we use other vayu components.
    // However, we are not using vayu::http::Client anymore.
