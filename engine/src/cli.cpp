...
int run_via_daemon (const std::string& daemon_url, const std::string& filepath, int verbosity, bool color) {
    // Read request file
    std::string content;
    try {
    ...
    bool is_load_test = json.contains ("mode") ||
    json.contains ("iterations") || json.contains ("duration");
    ...
    if (is_load_test) {
        ...
        if (res->status == 202) {
            ...
            // The live SSE stream (/runs/:id/live). Served
...
int run_cli (std::span<char* const> args) {
    ...
    // httplib uses its own socket implementation, but vayu::http::global_init
    // might be needed if we use other vayu components. However, we are not
