...
TEST_F (SseParserTest, FieldWithNoColonValue) {
    ...
    // Per SSE spec: "data" alone with no colon is equivalent to "data:" (empty
    // value) This appends empty string to data buffer, but since buffer was
    // empty, it remains empty and no event is dispatched (per spec)
    parser.feed ("data\n\n");
...
TEST_F (EventSourceTest, ConnectAndClose) {
    ...
    source.on_event (
    [&event_count] (const vayu::http::SseEvent&) { event_count++; });
...
TEST_F (EventSourceTest, TypedEventCallback) {
    ...
    source.on ("message",
    [&message_count] (const vayu::http::SseEvent&) { message_count++; });

    source.on (
    "custom", [&custom_count] (const vayu::http::SseEvent&) { custom_count++; });
...
TEST_F (EventSourceTest, ReceiveRealEvents) {
    ...
    source.on_event (
    [&event_count, &received_data, &data_mutex] (const vayu::http::SseEvent& event) {
        event_count++;
    ...
    if (event_count > 0) {
        ...
        std::cerr
        << "No events received (SSE test server may be unavailable)\n";
    }
}
