...
namespace vayu::utils
{
    ...
    public:
         ...
         */
        static DetailedReport calculate_detailed_report(const std::vector<vayu::db::Result> &results, double duration_s);
         ...
         */
        static nlohmann::json create_stop_response(const std::string &run_id, const RunSummary &summary);
         ...
         * @return JSON object with status, runId, and message.
         */
        static nlohmann::json create_already_stopped_response(const std::string &run_id, const std::string &status);
         ...
         */
        static bool wait_for_graceful_stop(vayu::core::RunContext &context, int timeout_seconds = 5);
    };
