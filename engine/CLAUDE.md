...
- **A behaviour attached to a request or collection at a phase of a step is
  ...
  **single-request** load run
  ...
  carries `title` and `description`, so a generated form has words to render
  instead of falling back to a raw property name - #1608 is what wires the
  app's own form to read them.
