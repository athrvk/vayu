...
- **MCP server for coding agents** - 69 typed tools over inspection, execution, load runs and writes, behind a host allowlist, load caps, and a write toggle that ships off (and withholds the write tools until it is on)
...
Windows (x64), macOS 13.3+ (universal), and Linux (AppImage). No account, no sign-in.
