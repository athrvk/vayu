...
import { recordToKeyValue, keyValueToRecord } from "./types";
...
export default function RequestBuilder() {
    ...
    const initialRequest = useMemo((): Partial<RequestState> | undefined => {
        ...
        return {
            ...
            params: recordToKeyValue(fetchedRequest.params || {}),
            headers: recordToKeyValue(fetchedRequest.headers || {}),
            bodyMode: fetchedRequest.body_type === "json" ? "json" :
                fetchedRequest.body_type === "form-data" ? "form-data" :
                    fetchedRequest.body_type === "x-www-form-urlencoded" ? "x-www-form-urlencoded" :
                        fetchedRequest.body_type === "text" ? "text" : "none",
            body: fetchedRequest.body || "",
            formData: [],
            urlEncoded: [],
            authType: fetchedRequest.auth?.type === "bearer" ? "bearer" :
                fetchedRequest.auth?.type === "basic" ? "basic" :
                    fetchedRequest.auth?.type === "api_key" ? "api-key" : "none",
    ...
    const handleExecute = useCallback(async (request: RequestState): Promise<ResponseState | null> => {
        ...
        try {
            ...
            const resolvedHeaders = Object.fromEntries(
                Object.entries(keyValueToRecord(request.headers)).map(([key, value]) => [
                    resolveString(key),
                    resolveString(value)
                ])
            );
            ...
            const resolvedAuthConfig = request.authType !== "none" && request.authConfig
            ...
            const engineRequest: Request = {
                ...
                body: resolvedBody,
                body_type: request.bodyMode === "json" ? "json" :
                    request.bodyMode === "form-data" ? "form-data" :
                        request.bodyMode === "x-www-form-urlencoded" ? "x-www-form-urlencoded" :
                            request.bodyMode === "text" ? "text" : undefined,
                auth: request.authType !== "none" ? {
                    type: request.authType === "bearer" ? "bearer" :
                        request.authType === "basic" ? "basic" :
                            request.authType === "api-key" ? "api_key" : "bearer",
                    ...resolvedAuthConfig,
                } : undefined,
            ...
            const result = await engineExecuteRequest(engineRequest, activeEnvironmentId || undefined);
            ...
            const bodyType: ResponseState["bodyType"] =
            ...
            return {
                ...
                body: typeof result.body === "object" ? JSON.stringify(result.body, null, 2) : String(result.body || ""),
                bodyType,
    ...
    const handleSave = useCallback(async (request: RequestState) => {
    ...
    const handleConfirmLoadTest = useCallback(async (config: LoadTestConfig) => {
        ...
        try {
            ...
            const resolvedHeaders = Object.fromEntries(
                Object.entries(keyValueToRecord(pendingLoadTestRequest.headers)).map(([key, value]) => [
                    resolveString(key),
                    resolveString(value)
                ])
            );
            ...
            const apiRequest: StartLoadTestRequest = {
                ...
                mode: config.mode === "constant_rps" ? "constant" :
                    config.mode === "constant_concurrency" ? "constant" :
                        config.mode === "iterations" ? "iterations" : "ramp_up",
                duration: config.duration_seconds ? `${config.duration_seconds}s` : undefined,
                targetRps: config.rps,
                iterations: config.iterations,
                concurrency: config.concurrency,
                rampUpDuration: config.ramp_duration_seconds ? `${config.ramp_duration_seconds}s` : undefined,
