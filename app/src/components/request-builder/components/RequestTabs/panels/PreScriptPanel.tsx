...
export default function PreScriptPanel() {
    ...
    const envGetPattern = /pm\.(?:environment|globals|collectionVariables)\.get\s*\(\s*['"]([^'"]+)['"]\s*\)/g;
    const templatePattern = /\{\{([^{}]+)\}\}/g;

    const envVars = [...request.preRequestScript.matchAll(envGetPattern)].map(m => m[1]);
    const templateVars = [...request.preRequestScript.matchAll(templatePattern)].map(m => m[1].trim());
        ...
        <div className="space-y-4">
            ...
            {hasReferencedVars && !showVariables && (
                <div className="flex flex-wrap gap-2">
                    ...
                    {usedVars.length > 5 && (
                        <span className="text-xs text-muted-foreground">+{usedVars.length - 5} more</span>
                    )}
                </div>
            )}
            ...
            <div className="text-xs text-muted-foreground space-y-1">
                ...
                <code className="block bg-muted p-2 rounded">
                    pm.environment.get("variable")<br />
                    pm.environment.set("key", "value")<br />
                    pm.globals.get("variable")<br />
