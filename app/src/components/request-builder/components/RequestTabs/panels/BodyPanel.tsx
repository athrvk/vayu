        ...
        <div className="space-y-4">
            ...
            {(request.bodyMode === "json" || request.bodyMode === "text") && (
                <div className={showPreview ? "grid grid-cols-2 gap-4" : ""}>
                    ...
                    {showPreview && (
                        <div className="space-y-2">
                            <label className="text-xs font-medium text-muted-foreground">Resolved Preview</label>
                            <pre className="h-[320px] p-3 border border-input rounded-md font-mono text-sm bg-muted/50 overflow-auto whitespace-pre-wrap">
                                {resolvedBody || <span className="text-muted-foreground italic">Empty body</span>}
                            </pre>
                        </div>
                    )}
                </div>
            ...
            {request.bodyMode === "form-data" && (
                <div className="space-y-2">
                    ...
                    <KeyValueEditor
                        items={request.formData.length > 0 ? request.formData : [createEmptyKeyValue()]}
            ...
            {request.bodyMode === "x-www-form-urlencoded" && (
                <KeyValueEditor
                    items={request.urlEncoded.length > 0 ? request.urlEncoded : [createEmptyKeyValue()]}
