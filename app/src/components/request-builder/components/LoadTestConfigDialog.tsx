			...
			<DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
				...
				<div className="space-y-4">
					...
					<div className="space-y-2">
						...
						<Select value={mode} onValueChange={(value) => setMode(value as LoadTestConfig["mode"])}>
							<SelectTrigger>
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
								<SelectItem value="constant_rps">Constant RPS (Requests per second)</SelectItem>
								<SelectItem value="constant_concurrency">Constant Concurrency</SelectItem>
					...
					{(mode === "constant_concurrency" || mode === "iterations" || mode === "ramp_up") && (
						<div className="space-y-2">
					...
					<div className="border-t pt-4 mt-4">
						<h3 className="text-sm font-semibold text-foreground mb-3">Data Capture Options</h3>

						<div className="space-y-3">
							<div className="space-y-2">
								<Label>Success Sample Rate (%) - Save {sampleRate}% of successful responses</Label>
								<input
									type="range"
									value={sampleRate}
					...
					<div className="p-3 bg-blue-50 border border-blue-200 rounded text-sm text-blue-800 dark:bg-blue-950 dark:border-blue-800 dark:text-blue-200">
						...
						{mode === "constant_rps" && (
							<p>Maintain {rps} requests/sec for {duration} seconds</p>
						)}
						{mode === "constant_concurrency" && (
							<p>Keep {concurrency} concurrent connections for {duration} seconds</p>
						)}
						{mode === "iterations" && (
							<p>Execute {iterations} requests with {concurrency} concurrent connections</p>
						)}
						{mode === "ramp_up" && (
							<p>Gradually increase to {concurrency} concurrent connections over {rampDuration}s, then maintain for {duration}s</p>
						)}
					</div>
				</div>
				...
				<DialogFooter>
					...
					<Button onClick={handleStart} disabled={isStarting} className="bg-purple-600 hover:bg-purple-700">
						{isStarting ? (
							<>
