...
export default function VariablesCategoryTree({
    ...
    const isSelected = (category: VariableCategory) => {
        ...
        if (category.type === 'collection') {
            return category.collectionId === (selectedCategory as { type: 'collection'; collectionId: string }).collectionId;
        }
        if (category.type === 'environment') {
            return category.environmentId === (selectedCategory as { type: 'environment'; environmentId: string }).environmentId;
        }
        return false;
    };
    ...
    const handleDeleteEnvironment = async (envId: string, e: React.MouseEvent) => {
        ...
        if (selectedCategory?.type === 'environment' &&
            (selectedCategory as { type: 'environment'; environmentId: string }).environmentId === envId) {
            ...
            <div className="mb-4">
                <button
                    ...
                    className={cn(
                        ...
                        isSelected({ type: 'globals' }) && "bg-primary/10 text-primary hover:bg-primary/15"
                    )}
                >
                            ...
                            environments.map((environment) => {
                                ...
                                return (
                                    <div
                                        key={environment.id}
                                        className={cn(
                                            "group flex items-center gap-2 px-3 py-2 pl-6 text-sm hover:bg-accent transition-colors cursor-pointer",
                                            isSelected({ type: 'environment', environmentId: environment.id }) &&
                                    ...
                                    >
                                        ...
                                        {variableCount > 0 && (
                                            <Badge variant="secondary" className="text-xs bg-blue-100 text-blue-600 dark:bg-blue-950 dark:text-blue-300 px-1.5 py-0 shrink-0">
                                                {variableCount}
                                            </Badge>
                                        )}
                                        <Button
                                            variant="ghost"
                                            size="icon"
                                            onClick={(e) => handleDeleteEnvironment(environment.id, e)}
                                            disabled={isDeleting}
                            ...
                            collections.map((collection) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={cn(
                                            "w-full flex items-center gap-2 px-3 py-2 pl-6 text-left text-sm hover:bg-accent transition-colors",
                                            isSelected({ type: 'collection', collectionId: collection.id }) &&
                                    ...
                                    >
                                        ...
                                        {variableCount > 0 && (
                                            <Badge variant="secondary" className="text-xs bg-orange-100 text-orange-600 dark:bg-orange-950 dark:text-orange-300 px-1.5 py-0">
                                                {variableCount}
                                            </Badge>
                                        )}
                                    </button>
