 ...
 *
 * Phase 0 ships a bespoke form only for `script.pre` / `script.post`, so a
 * script keeps its Monaco editor rather than becoming a plain text field.
 * `extract.json`'s path field is deliberately generic-form only for now (a
 * bespoke response-body picker is real UI work with no engine dependency,
 * so it is a follow-up rather than part of this cut).
 */
