...
import { Card, CardContent, CardHeader, CardTitle, Badge, Button, ScrollArea } from "@/components/ui";
            ...
            <Card>
                ...
                <CardContent>
                    {hasStatusCodes ? (
                        ...
                        <p className="text-sm text-muted-foreground">No status code data available</p>
                    )}
                </CardContent>
            </Card>
                                        ...
                                        <div key={index} className="border-b last:border-b-0">
                                            ...
                                            >
                                                <div className="flex items-center gap-3 w-full">
                                                    ...
                                                    {/* Status Code */}
                                                    <Badge
                                                        variant={getStatusBadgeVariant(result.statusCode)}
                                                    ...
                                                    <span className={cn(
                                                        ...
                                                        isSlow && "text-orange-600 dark:text-orange-400"
                                                    )}>
                                                        ...
                                                        <>
                                                            {result.trace.error_type && (
                                                                <div className="flex gap-4 text-sm">
                                                                    ...
                                                                    <span className="font-mono">{result.trace.error_type}</span>
                                                                </div>
                                                            )}

                                                            {/* Timing Breakdown - using camelCase field names from backend */}
                                                            {(result.trace.dnsMs !== undefined ||
                                                                result.trace.connectMs !== undefined ||
                                                                result.trace.tlsMs !== undefined ||
                                                                result.trace.firstByteMs !== undefined ||
                                                                result.trace.downloadMs !== undefined) && (
                                                                    <div className="space-y-1">
                                                                        <p className="text-xs font-medium text-muted-foreground">Timing Breakdown</p>
                                                                        <div className="grid grid-cols-5 gap-2 text-xs">
                                                                            {result.trace.dnsMs !== undefined && (
                                                                                <div className="bg-muted p-2 rounded text-center">
                                                                                    <p className="text-muted-foreground">DNS</p>
                                                                                    <p className="font-mono font-medium">{result.trace.dnsMs.toFixed(1)}ms</p>
                                                                                </div>
                                                                            )}
                                                                            {result.trace.connectMs !== undefined && (
                                                                                <div className="bg-muted p-2 rounded text-center">
                                                                                    <p className="text-muted-foreground">Connect</p>
                                                                                    <p className="font-mono font-medium">{result.trace.connectMs.toFixed(1)}ms</p>
                                                                                </div>
                                                                            )}
                                                                            {result.trace.tlsMs !== undefined && (
                                                                                <div className="bg-muted p-2 rounded text-center">
                                                                                    <p className="text-muted-foreground">TLS</p>
                                                                                    <p className="font-mono font-medium">{result.trace.tlsMs.toFixed(1)}ms</p>
                                                                                </div>
                                                                            )}
                                                                            {result.trace.firstByteMs !== undefined && (
                                                                                <div className="bg-muted p-2 rounded text-center">
                                                                                    <p className="text-muted-foreground">TTFB</p>
                                                                                    <p className="font-mono font-medium">{result.trace.firstByteMs.toFixed(1)}ms</p>
                                                                                </div>
                                                                            )}
                                                                            {result.trace.downloadMs !== undefined && (
                                                                                <div className="bg-muted p-2 rounded text-center">
                                                                                    <p className="text-muted-foreground">Download</p>
                                                                                    <p className="font-mono font-medium">{result.trace.downloadMs.toFixed(1)}ms</p>
                                                                                </div>
                                                                            )}
                                                                        </div>
                                                                    </div>
                                                            ...
                                                            {result.trace.isSlow && (
                                                                <div className="flex items-center gap-2 text-xs bg-orange-50 dark:bg-orange-950/30 text-orange-700 dark:text-orange-400 p-2 rounded">
                                                                    ...
                                                                    <span>
                                                                        Slow request: {result.trace.totalMs?.toFixed(1)}ms
                                                                        {result.trace.thresholdMs && (
                                                                            <span className="text-muted-foreground ml-1">
                                                                                (threshold: {result.trace.thresholdMs}ms)
                                                                            </span>
                                                                        )}
                                                                    </span>
                                                                </div>
                                                            ...
                                                            {result.trace.headers && Object.keys(result.trace.headers).length > 0 && (
                                                                <div className="space-y-1">
                                                                    ...
                                                                    <div className="bg-muted p-2 rounded text-xs font-mono max-h-32 overflow-auto">
                                                                        {Object.entries(result.trace.headers).map(([key, value]) => (
