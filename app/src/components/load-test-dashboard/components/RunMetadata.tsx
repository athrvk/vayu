            ...
            <div className="flex flex-wrap items-center gap-4 text-sm">
                ...
                {requestUrl && (
                    <div className="flex items-center gap-2">
                        ...
                        {requestMethod && (
                            <Badge variant="outline" className={cn("text-xs font-bold", getMethodColor(requestMethod))}>
                                {requestMethod}
                            </Badge>
                        )}
                        <span className="text-foreground font-mono text-xs truncate max-w-md" title={requestUrl}>
                            {requestUrl}
                        </span>
                    </div>
                )}
                ...
                {configuration?.mode && (
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-foreground capitalize">{configuration.mode.replace('_', ' ')}</span>
                        {configuration.targetRps && (
                            <span className="text-muted-foreground">@ {configuration.targetRps} RPS</span>
                        )}
                        {configuration.concurrency && (
                            <span className="text-muted-foreground">({configuration.concurrency} concurrent)</span>
                        )}
                    </div>
                )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 text-sm mt-2">
                ...
                {(elapsedDuration > 0 || startTime) && (
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-foreground">
                            {mode === "completed"
                                ? formatDuration(elapsedDuration || (endTime && startTime ? endTime - startTime : 0))
