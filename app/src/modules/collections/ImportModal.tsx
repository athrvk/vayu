...
export function ImportModal() {
  ...
  const detect = (raw: string, opts: { importEnvironments: boolean; importScripts: boolean }, fileName?: string) => {
    try {
      ...
      setResult(null);
      setError(e instanceof UnrecognisedFormatError ? "Unrecognised format" : (e as Error).message);
      setPhase("error");
    }
  };
            ...
            <>
              {tab === "file" && (
                ...
                >
                  ...
                  <p className="mt-2 text-[13px] font-medium">Drop a file here, or click to browse</p>
                  <p className="text-[11px] text-muted-foreground">Format is detected automatically</p>
                  <input
                    ref={fileInputRef}
                    type="file"
                    className="hidden"
                  ...
                  <div className="mt-4 flex flex-wrap justify-center gap-1.5">
                    {FORMAT_BADGES.map((b) => (
                      <span key={b} className="rounded border border-border bg-card px-2 py-0.5 text-[10px] font-semibold">{b}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {phase === "error" && (
                <p className="mt-3 text-[12px]" style={{ color: "var(--destructive-text)" }}>{error}</p>
              )}
            </>
          )}
        </div>
          ...
          <div className="flex items-center justify-between gap-3 border-t border-border px-5 py-4">
            <label className="flex flex-col gap-1 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1.5">
                <input type="checkbox" checked={importEnvironments} onChange={(e) => toggleEnvironments(e.target.checked)} />
                Import environments &amp; variables
              </span>
              <span className="flex items-center gap-1.5">
                <input type="checkbox" checked={importScripts} onChange={(e) => toggleScripts(e.target.checked)} />
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        {meta.fileName && <span className="font-mono text-[11px] text-muted-foreground">{meta.fileName}</span>}
        <button className="ml-auto text-muted-foreground hover:text-foreground" onClick={onDismiss} aria-label="Dismiss">
      ...
      <p className="text-[11px] text-muted-foreground">
        {meta.requestCount} requests · {meta.folderCount} folders · {meta.environmentCount} environments
      </p>
      {(meta.skipped.length > 0 || meta.nonExecutableAuth > 0) && (
        <p className="flex items-center gap-1.5 text-[11px]" style={{ color: "var(--destructive-text)" }}>
...
function TreeNode({
  ...
  return (
    <div className="pl-1">
      ...
      <div className="pl-5">
        {requests.map((r, i) => (
          <div key={i} className="flex items-center gap-2 py-0.5 text-[11px] text-muted-foreground">
