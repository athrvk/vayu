 ...
 *
 * This file used to guard the same lifetime for a second thing kept up here
 * for the same reason: which Content-Type row a body mode added, so that
 * leaving the mode could remove it. That record is gone (issue #1481) - the
 * row now carries its own ownership (`KeyValueEntry.source`), which needs no
 * provider lifetime at all, and is tested as logic in `content-type.test.tsx`.
 */
