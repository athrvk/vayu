...
function pmRequest(item: any, ctx: Ctx): RequestDraft {
  ...
  return {
    ...
    description: typeof rq.description === "string" ? rq.description : rq.description?.content ?? "",
...
function pmFolder(node: any, ctx: Ctx): CollectionDraft {
  ...
  return {
    ...
    preRequestScript: ctx.opts.importScripts ? joinExec(events.find((e) => e.listen === "prerequest")) : "",
    postRequestScript: ctx.opts.importScripts ? joinExec(events.find((e) => e.listen === "test")) : "",
    children,
    requests,
  };
}
...
function parsePostman(parsed: any, opts: ImportOptions, formatName: string): ImportResult {
  const ctx: Ctx = { opts, requestCount: 0, folderCount: 0, nonExecutableAuth: 0, skippedFileBody: 0 };
