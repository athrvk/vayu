 ...
 *        `vayu://elements/kinds` re-serves the engine's element registry
 *        catalogue the same way (issue #1517), so an agent building an
 *        `elements` list reads the current kinds and config schemas rather
 *        than guessing them.
...
export const STATIC_RESOURCES: StaticResourceDef[] = [
	...
	},
	{
		...
		description:
			...
			"a script.pre / script.post element's script (or a create_request/update_request " +
			"preRequestScript/postRequestScript, still accepted as sugar for one) - it is generated " +
			"by the engine, so it is current for the running version.",
	...
	{
		name: "elements",
		uri: "vayu://elements/kinds",
		title: "Element catalogue",
		// The registry, not a prose copy of it - the same reason the two scripting
		// resources above re-serve the engine's own list rather than restating it:
		// a kind's config shape changes as the engine adds kinds (issue #1512), and
		// a hardcoded description would drift the moment it did.
		description:
			"Every registered element kind an agent can add to a request or collection's " +
			"`elements` list - script.pre/script.post, extract.*, assert.*, timer.think and " +
			"more - with its category, the phase(s) it runs at, its config's JSON Schema, and " +
			"a human label and description. Read this before writing an `elements` entry blind.",
		read: async (ctx, signal) => projectElementKinds(await ctx.client.getElementKinds(signal)),
	},
];
...
/** One catalogue entry from `GET /elements/kinds` (issue #1513) - a kind's
 *  identity plus what a request or collection's `elements` list needs to
 *  configure and place one; extra fields the engine adds later ride along
 *  under `Record<string, unknown>` rather than being dropped. */
export interface ElementKindEntry extends Record<string, unknown> {
	kind: string;
}

/**
 * Reduce `GET /elements/kinds` to its kind entries, dropping anything
 * malformed rather than trusting an object with no `kind` string.
 *
 * Throws on a non-array payload or on an array with no valid entry, for the
 * reason {@link projectScriptingSurface} throws: an agent reading an empty
 * catalogue concludes no element kinds exist and falls back to a raw script
 * for work a kind already covers.
 */
export function projectElementKinds(payload: unknown): ElementKindEntry[] {
	if (!Array.isArray(payload)) {
		throw new Error(
			"GET /elements/kinds did not return an array - cannot describe the element catalogue"
		);
	}
	const kinds: ElementKindEntry[] = [];
	for (const item of payload) {
		if (
			...
			kinds.push(item as ElementKindEntry);
		}
	}
	if (kinds.length === 0) {
		throw new Error(
			"GET /elements/kinds returned no kind entries - cannot describe the element catalogue"
		);
	}
	return kinds;
}
