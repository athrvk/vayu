...
For the same reason, **dynamic variables are not readable from a script.**
`{{$guid}}`, `{{$timestamp}}` and the rest of the set in
[variable resolution](../app/variable-resolution.md#dynamic-variables) are
generated by the app while it interpolates the payload; by the time a script
runs, that payload holds the generated *value* and no scope has ever heard of
the name. `pm.variables.get("$guid")` reads as any other undefined name does.
