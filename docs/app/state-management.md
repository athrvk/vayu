...
Once a run reaches a terminal status its stored `results` rows are the complete record and the view reads those instead - `ScenarioRunService.handleClose` invalidates `runs.detail(runId)` and fetches `runs.report(runId)` through the query cache (the same keys the tab reads, so a bare fetch would leave the pane on the stale copy).
