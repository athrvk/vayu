# Task 3a — Create `tabs-store.ts` with full tab model

## Context

Repository: `athrvk/vayu`. Work on branch `claude/clever-sagan-b4oTu`.

This task creates the central tab store that replaces the `navigation-store`'s
`activeScreen` + `tabMemory` + `resolveActiveScreen` system.
**Do not touch `navigation-store` yet** — that dissolution happens in task 6b.
This task only adds the new store.

## Tab type definitions

**New file**: `app/src/stores/tabs-store.ts`
...
- `pnpm lint` passes.
- The store is exported from the barrel.
- Singleton types dedupe correctly (calling `openTab({ type: "settings", entityId: null })` twice only results in one tab and focuses it).
- LRU eviction: adding a 13th tab removes the oldest non-active, non-dashboard tab.

## Files to touch

- `app/src/stores/tabs-store.ts` (new)
