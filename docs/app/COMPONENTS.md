...
`CollectionDetail/SpecTab.tsx` is the same two-halves split one contract over (issue #638, phase 1b of #625). The **document** and the binding (`collection.openapi` -> the engine's `spec_documents`) are the same on every machine and ride the engine; a picked file's **path** is true of one filesystem and lives in `stores/spec-file-store.ts`, which holds a path and a file name and no spec content at all. Binding a collection that was not imported from a spec goes through `services/openapi/spec-operations.ts` - which reads the operations back out through the **import parsers** rather than walking `paths` a second time, so the two cannot disagree about what a document contains - and `POST /specs/match`, which pairs them - the matcher moved into the engine in issue #761 so an agent over MCP can bind a collection the same way, and it reduces both sides to a path shape (origin, query and fragment dropped; `{{petId}}` and `{petId}` flattened alike) and refuses an ambiguous pair rather than guessing. `services/openapi/operation-match.ts` keeps only that reduction, for the spec diff and the export skeleton, pinned against the engine's copy by `operation-shape.conformance.test.ts`. The bind itself is one call (`useBindSpecMutation` in `queries/specs.ts` -> `POST /specs/bind`, issue #862): the document, the binding and both halves of the stamping - what matched, and what the new document no longer accounts for - commit together, and the pairing is worked out engine-side from the bytes it stores. It was three ordered writes from the renderer until then, which is three places for a bind to stop and, in the clearing half, a list a caller could forget (issue #718). The user-facing contract lives in [OpenAPI Collections](openapi.md).
