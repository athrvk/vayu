...
`lib/settings-index.ts` is a **pure** module: `buildSettingsIndex` flattens the app panels and the engine entries into one list, `searchSettings` filters and ranks it (label, then key, then description; an empty query means "not searching" and returns everything). It is deliberately free of React and stores so the sidebar's search box and the command palette's settings source share one corpus. A result selects the owning category and, for an engine entry, passes its key as `useSettingsStore.highlightedKey`; `SettingsMain` scrolls that card into view, outlines it, uncollapses **Advanced** if the entry lives there, and clears the key once acted on.
...
- `ToggleRow` - label (a node, for the MCP tool rows' `<code>` and counts), description, `disabled`, `title`.
