...
|---|---|
| Variables used (`variables`) | `VariablesSection.tsx` - the variables this request references, resolved or not, with the full in-scope set behind a disclosure and a quick editor over them (below). The `id` stays `variables` - it keys the collapsed state. |
