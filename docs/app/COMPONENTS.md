...
> **Choosing GraphQL on a default GET also moves the method, and leaving GraphQL puts it back** (issue #1228). A new request is a GET, and GraphQL-over-HTTP means something different on one - the document travels as query parameters, and a mutation cannot be sent that way at all - so picking the mode used to build a request the server answered with a bare `400`. The mode now sets `POST` the same reversible way it sets the header above, but through a **provider-held slot** rather than a marker on a row: what it owns is a scalar field, not a header row, so it has nowhere to carry a marker of its own and instead remembers the value it wrote (`method`) and what it replaced (`previous`). `switchGraphQLMethod` in `panels/body/graphql-method.ts` is the rule - only from GET, only to POST, and a method the user chose is never overridden in either direction, the same "no longer ours" check `switchAutoHeader` makes on an edited row. A GET still reaches GraphQL by another door - the user picks it back, or an import wrote one - so the Query pane header carries a `BadgeText` reading "Sent as query parameters" whenever the method is GET, naming the transport rather than reporting an error: GET is a legitimate way to send a query, and the wrong one for a mutation. The record lives in `RequestBuilderProvider` (see [state-management](state-management.md#requestbuildercontext---the-headers-a-setting-added)) because the panel is unmounted whenever another tab is on screen - it has the same reload gap the Content-Type row had, tracked as issue #1505 since `method` has no row to mark.
