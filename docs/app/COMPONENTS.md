...
**A dirty draft can show "Changed elsewhere" instead of losing an edit (#1437).** An MCP agent's `update_collection` landing while Info, a Script tab or Auth is mid-edit no longer overwrites the unsaved text: `shared.tsx`'s `ExternalChangeCallout` renders the notice with a "Take theirs" action, driven by `useEntityDraft`'s `externalValue`. Auth and the Script tabs treat their draft as one value, so the whole tab shows one callout wired to `reset()`. Info's draft is `{name, description}`, so it merges per field instead - a field the user has not touched adopts the agent's change immediately, and only a field both sides touched shows its own callout.
