...
Main layout: tab-centric with an activity rail, resizable drawer, split/overlay context bar with its own rail, and a status-only docked footer.
...
Footer bar (h-8, shrink-0), ambient status only since #1615 moved its two switchers onto the rails above. Every item here already has a non-footer path when there is something to click, so the system Dock covering this strip on macOS costs a glance, never a click.
