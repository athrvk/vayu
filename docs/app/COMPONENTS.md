...
**It scrolls the pane's own scroller, never `scrollIntoView` on the anchor** (#1612). Both panels wrap their cards in a `[data-settings-scroller]` div nested inside `overflow-hidden` columns (the pane itself and the shell around it); `Element.scrollIntoView` walks *every* scrollable ancestor until the target is satisfied, and Chromium counts `overflow: hidden` as scrollable for that purpose even though no user gesture can move it back - so a call that needed more travel than the inner scroller had left pushed the category header (with Reset to Defaults / Save Changes) out of the pane for good. `scrollWithin` (`@/lib/scroll-within`) instead finds the nearest `[data-settings-scroller]` ancestor and scrolls only that container, computed from `getBoundingClientRect`. An anchor with no such ancestor is not scrolled at all, rather than falling back to a guess. `SchemaExplorer`'s own reveal-on-search uses the same helper against its own list container, for the identical reason.
