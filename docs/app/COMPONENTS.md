...
**Density scales rhythm, not chrome, interactive targets or icons** (issue #1679, superseding #1676's "the shrink applies to icons too" decision). A chrome band (the tab strip, drawer header, notification banners), an interactive target (a button, a checkbox, a toggle) and an icon each carry their own floor - seven named steps outside the `--spacing` multiplier, `--spacing-band`/`-banner`/`-control`/`-control-sm`/`-target`/`-icon`/`-icon-sm` in `index.css`'s `@theme inline` block - so Comfortable no longer means "the tab strip and the toggle switches get taller too" the way #1670's single unit implied. See [Chrome, Target and Icon Floors](../design-system.md#chrome-target-and-icon-floors) for the values and which components read which step, and [Spacing Scale Conventions](../design-system.md#spacing-scale-conventions) for the rhythm unit itself.
