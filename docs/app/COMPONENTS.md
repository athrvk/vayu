...
That catalogue is a second copy of what the panels render, so it is checked rather than trusted: `anchor` is the `data-setting-anchor` the panel puts on the block holding the setting, and `app-settings.drift.test.tsx` renders every panel and fails if a declared anchor is not on screen. Rendered, not source-scanned - `NumberSettingRow` takes its anchor as a prop, which no scan of a panel file would see.
