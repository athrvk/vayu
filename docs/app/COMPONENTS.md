...
The **Custom metric budgets** rows inside it (issue #1579, the same `CustomBudgetRows.tsx` the load dialog mounts) are the one exception, and are gated by Load test: `custom.<name>.<stat>` is evaluated for load runs only - a collection run's config accepts the block and nothing evaluates it yet (`docs/engine/api-reference.md`, the thresholds block) - so with the switch off the rows are replaced by a sentence saying which switch turns them on, rather than taking a budget the run would never be judged against. Rows already typed are kept in state across a toggle, and while the switch is off they neither reach the payload nor block Run.
