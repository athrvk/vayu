...
| Spec | `SpecTab.tsx` | The bound **OpenAPI document** (issue #638): where it came from, its short hash, when it was fetched and bound, and how many of the subtree's requests carry an operation identity. Binds a collection that was not imported from a spec - match by method and path shape, counts disclosed, matches stamped and nothing else touched - and **Unbind** sends `openapi: null`. The **Sync** section (`SpecSync.tsx`, issue #654) re-reads the document from the URL or file the binding recorded and reports what moved in three buckets - it writes nothing. Explicit per action, so it is absent from `TABS_HOLDING_DRAFTS` like Data |
