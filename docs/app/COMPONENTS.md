...
**The list is bounded in cost, not in content** (issue #730). `maxScenarioStoredSteps` is 5,000 and every row used to mount as a card. The four count chips are now buttons that filter the list to one outcome - the reader who has just read "11 failed" wants those eleven, and the number is where they read it - and the rows arrive through `useGrowingWindow`, the same growing list the response pane's console output uses, each card marked `skip-offscreen` so the ones scrolled past cost no layout. No virtualiser, for the reason that hook records: step cards vary in height, and variable-height windowing means a scroll map to maintain forever. Filtering to an outcome whose *stored* rows are all gone (thinning drops passes) says so in its empty state rather than reading as a contradiction with the chip, which counts the run.

**A search beside the chips, matching the step name** (issue #832). The outcome filter cannot answer "show me every execution of `POST /checkout`": a 5,000-step run of a 40-request collection is 40 names repeated 125 times, and the failures a chip narrows to are spread across a dozen different steps. The box matches the **step name**, stated on the field itself (`Search step names`) rather than left to be guessed - it is the one field both sources carry, where a row's URL lives in its stored trace and a live row has none, so matching that would make the same box search less while a run streams than after it ends. The two controls are **one predicate** (`filterSteps` in `scenario-steps.ts`), so narrowing by both is the useful case rather than a special one, and there is a single empty state (`emptyStepListReason`) that names whichever combination emptied the list - the chip and the search are cleared in different places, and a reader told "no failed steps" over a list their search emptied clears the wrong control. The outcome half of that message keeps the thinning disclosure it always carried. The growing window is keyed on the shown total, so a narrowed list starts at its own top.
