...
Every request records which operation it is - `{ operationId?, method, path }`, the templated path exactly as the document writes it (`/pets/{petId}`), never the `{{petId}}` rewrite that goes into the URL. `parse` claims one identity per operation through `createOperationIdentifier(tally)` (`openapi-shared.ts`) and stamps it on the request draft. The **declared-operation index** stored beside the document is no longer built here: the engine reads the document as it stores it and derives the index itself (issue #853), so one reader answers what a document declares. What the two still owe each other is agreement - coverage matches a request's stamp against that index - and `engine/tests/fixtures/declared-operations-conformance.json` is the table both suites read to keep it.

The **request itself** is now mirrored engine-side as well (issue #865, `core::spec_request_drafts_of`): everything this page describes - the URL and its joined query, the parameter rows and their enabled state, the sampled body, the folder an operation is filed under - the engine derives from the stored document too, because the [sync diff](../openapi.md#checking-a-bound-spec-for-changes) compares a stored request against one of those and is moving down with it. Nothing here is deleted for it: this parser is still what an import runs, and the two are held to the same answers by `engine/tests/fixtures/spec-request-drafts-conformance.json`, read by `spec-request-drafts.conformance.test.ts` here and `openapi_drafts_test.cpp` there. A rule changed on this page therefore has to change on both sides, or the diff starts reporting a field as changed for a document that did not change.
