...
**`TabCount` / `TabErrorDot`** (`app/src/components/ui/tabs.tsx`) are the answer for the data-derived case `LabelSwap` turns away. A mark beside a tab label is a flex item on a `shrink-0` trigger inside a `flex-nowrap` list, so one that *mounts* widens its trigger by its own width plus the trigger's `gap-1.5` and pushes every trigger to its right - and whatever else shares the row - along with it; typing the first character into an empty Params table moved the seven tabs after Params and the `Table` toggle past them. A hidden widest twin cannot fix it, because a count is a number rather than one string rendered two ways. So the slot is held open instead and the mark empties rather than unmounting: both components carry `MARK_SLOT`, `min-w-[1ch]` in the count's own `font-mono` `text-micro` (one `ch` there *is* one digit, at whatever the font scale resolves it to, and every digit has the same advance), and `TabCount` renders an empty `<sup>` when there is nothing to count - empty, so it reaches neither the eye nor the accessible name, which is how "a zero renders nothing" survives the slot becoming permanent. The fade moves onto an inner span, since `.enter-fade` needs a mount and the slot no longer has one. Two consequences for call sites: render `TabCount` **unconditionally** wherever a tab can carry a count, passing `undefined` for none (gating the element is what reintroduces the shift), and render none at all on a tab that never can, so it pays no reserved width. `min-w`, not `w`: a count crossing 9 to 10 is content genuinely growing and still moves its neighbours by one digit - left that way on purpose, because reserving two digits costs that width permanently on strips that already scroll at seven and eight tabs.
