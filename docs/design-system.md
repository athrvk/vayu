...
Two things a caller has to know. **`overflow-hidden` is permanent while the section is open, not just while it animates**, so this box does clip an outset focus ring - and it deliberately does *not* answer that with `.panel-clip`. The same `Input`, `Switch` or row-enable checkbox renders both inside a disclosure and outside one, so tucking the ring inward here would give one control two looks depending on where it sits: the clearance-over-tucking rule below, which `key-value-parity.test.tsx` already guards for the checkbox. A consumer whose control sits flush against the box's left, right or bottom edge adds clearance of its own - `px-3 py-3`, the way the load-test dialog's disclosures do it. **And a consumer that unmounts its children on the same state the `Collapsible` is open on** gets the opening half and not the closing one. `{expanded && children}` is deliberate in `context-bar/Section.tsx`, `VariablesSection.tsx` and `ScriptSnippets.tsx` (a collapsed section must run no queries), but Radix re-measures the box after React has already removed those children, so the closing keyframe runs from nothing: the content vanishes at once and only the empty box waits out the 200ms. Opening is unaffected, because the children mount in the same commit that flips the state. Keep the guarantee; the closing half is its price, not a bug in either piece.
