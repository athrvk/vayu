...
| A pre-request/post-response script | **Script**, living under the **Elements** tab | Keep "Elements" as the tab name (the consolidation from separate Pre-request/Tests tabs was deliberate), but individual script errors/labels keep saying "Pre-request Script" / "Post-response Script" - renaming those to "Elements" would lose information the label carries. |
