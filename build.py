...
# Every directory under engine/ that a build can generate, relative to it.
# `-c` removes all of them rather than only the one the current preset writes
# to: a dev clean that leaves engine/build-release standing is not a clean.
# Engine *runtime* state (engine/data, engine/db) is deliberately absent - that
# is the daemon's saved data, not a build artifact.
ENGINE_BUILD_DIRS = ("build", "build-release", "build-test", "out")
...
def build_engine(preset: str, clean: bool, run_tests: bool, project_root: Path) -> Optional[Path]:
    ...
    # Clean - every generated engine directory, not just this preset's. The
    # release tree is as stale after a dev clean as the debug one is.
    if clean:
        stale = [engine_dir / name for name in ENGINE_BUILD_DIRS
                 if (engine_dir / name).exists()]
        if stale:
            spinner = Spinner("Cleaning engine build directories")
            spinner.start()
            for path in stale:
                shutil.rmtree(path)
            spinner.stop()
...
def build_app(dev_mode: bool, engine_binary: Optional[Path], project_root: Path, clean: bool = False) -> bool:
    ...
    # Clean - the app's counterpart to wiping the engine's build directories.
    #
    ...
    #
    # `clean:app` removes whole generated directories (dist, dist-electron,
    # release, build) and never individual files inside one. Enumerating files
    # in a generated directory is what let app/build survive a `-c` before:
    # the list named five paths under it and missed everything else, including
    # a `build/icon.icns.png` that is never written in the first place.
    #
    # This runs after the install (rimraf lives in node_modules) and before
    # setup_icons, which recreates build/ with the icons in it.
    if clean:
